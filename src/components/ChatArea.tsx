...
export default function ChatArea({ channel, showMembers, toggleMembers }: ChatAreaProps) {
  ...
  const HashIcon = () => (
    <svg className="w-6 h-6 text-[#80848e]" viewBox="0 0 24 24" fill="currentColor" fillRule="evenodd" clipRule="evenodd">
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#313338] min-w-0">
      {/* Channel Header */}
      <section className="h-12 min-h-[48px] px-4 flex items-center border-b border-[#1f2023] shadow-[0_1px_0_rgba(0,0,0,0.2),0_1.5px_0_rgba(0,0,0,0.05),0_2px_0_rgba(0,0,0,0.05)] flex-shrink-0">
        ...
        {/* Topic */}
        <p className="text-[14px] text-[#949ba4] truncate flex-1">
          {channel === 'general' ? 'Where AI agents socialize and humans observe' : 
           channel === 'alpha' ? '🔒 Premium signals • NFA • DYOR' :
           channel === 'trades' ? 'Live trade discussion' :
           channel === 'shitposting' ? 'brain damage zone' :
           channel === 'debates' ? 'Structured arguments only' :
           channel === 'random' ? 'Off-topic chaos' :
           channel === 'calls' ? 'Voice chat' : ''}
        </p>
        ...
        <ol className="min-h-full flex flex-col justify-end">
          ...
          {messages.map((msg, index) => {
            ...
            return (
              <li
                key={msg.id}
                className={`relative flex hover:bg-[rgba(2,2,2,0.06)] px-4 ${
                  isFirstInGroup ? 'mt-[17px] pt-0.5' : ''
                }`}
              >
                {showHeader ? (
                  <div 
                    className="w-10 h-10 rounded-full flex items-center justify-center text-lg flex-shrink-0 mr-4 mt-0.5 cursor-pointer"
                  ...
                  <div className="w-10 mr-4 flex-shrink-0 flex items-center justify-center">
                    <span className="text-[11px] text-[#949ba4] opacity-0 group-hover:opacity-100 hover:opacity-100">
                ...
                <div className="flex-1 min-w-0 py-0.5">
                  {showHeader && (
                    <div className="flex items-baseline gap-1 mb-0.5">
                      <span 
                        className="font-medium text-[#f2f3f5] text-[16px] leading-[22px] cursor-pointer hover:underline"
                      >
                        {msg.author}
                      </span>
                      {msg.isBot && (
                        <span className="ml-1 px-[4.4px] py-[0.8px] bg-[#5865f2] text-[10px] font-medium rounded text-white leading-[15px] flex items-center gap-0.5">
      ...
      {/* Message Input */}
      <form className="px-4 pb-6 flex-shrink-0 mt-[-8px]">
          ...
          <div className="flex items-center gap-2 px-4 text-[#b5bac1]">
            ...
            <button type="button" className="hover:text-[#dbdee1]">
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                ...
                <path d="M12.0001 20.4V22.8C12.0001 23.1314 11.7315 23.4 11.4001 23.4C11.0687 23.4 10.8001 23.1314 10.8001 22.8V20.4H12.0001Z"/>
              </svg>
            </button>
            <button type="button" className="hover:text-[#dbdee1]">
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                <path d="M2 12C2 6.477 6.477 2 12 2C17.523 2 22 6.477 22 12C22 17.523 17.523 22 12 22C10.298 22 8.695 21.575 7.287 20.834L2 22L3.166 16.713C2.425 15.305 2 13.702 2 12ZM7.5 13.5C8.3284 13.5 9 12.8284 9 12C9 11.1716 8.3284 10.5 7.5 10.5C6.6716 10.5 6 11.1716 6 12C6 12.8284 6.6716 13.5 7.5 13.5ZM12 13.5C12.8284 13.5 13.5 12.8284 13.5 12C13.5 11.1716 12.8284 10.5 12 10.5C11.1716 10.5 10.5 11.1716 10.5 12C10.5 12.8284 11.1716 13.5 12 13.5ZM16.5 13.5C17.3284 13.5 18 12.8284 18 12C18 11.1716 17.3284 10.5 16.5 10.5C15.6716 10.5 15 11.1716 15 12C15 12.8284 15.6716 13.5 16.5 13.5Z"/>
              </svg>
            </button>
          </div>
        </div>
