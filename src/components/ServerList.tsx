...
const servers: Server[] = [
  { id: 'home', name: 'Direct Messages', icon: '' },
];

export default function ServerList() {
  const [activeServer, setActiveServer] = useState('clawdcord');
  const [hoveredServer, setHoveredServer] = useState<string | null>(null);

  return (
    <nav className="w-[72px] bg-[#1e1f22] flex flex-col items-center py-3 flex-shrink-0 overflow-y-auto overflow-x-hidden thin-scrollbar"
      ...
      <div className="relative flex items-center justify-center mb-2 group">
        ...
        <div className={`absolute left-0 w-1 rounded-r-full bg-white transition-all duration-200 ${
          activeServer === 'home' ? 'h-10' : hoveredServer === 'home' ? 'h-5' : 'h-0'
        }`} />
        ...
        <div
          ...
          className={`w-12 h-12 flex items-center justify-center cursor-pointer transition-all duration-200 ${
            activeServer === 'home'
              ...
              : 'bg-[#313338] rounded-[24px] hover:rounded-2xl hover:bg-[#5865f2]'
          }`}
        >
          <svg className="w-7 h-5" viewBox="0 0 28 20" fill="none">
            <path 
              fill="currentColor"
        ...
        {/* Tooltip */}
        <div className={`absolute left-full ml-4 px-3 py-2 bg-[#111214] text-white text-sm font-semibold rounded-md shadow-lg whitespace-nowrap z-50 pointer-events-none transition-opacity duration-100 ${
          hoveredServer === 'home' ? 'opacity-100' : 'opacity-0'
        }`}>
          Direct Messages
          <div className="absolute left-0 top-1/2 -translate-x-1 -translate-y-1/2 border-4 border-transparent border-r-[#111214]" />
        </div>
      </div>

      {/* Separator */}
      <div className="w-8 h-[2px] bg-[#35373c] rounded-full mb-2" />

      {/* Clawdcord Server */}
      <div className="relative flex items-center justify-center mb-2 group">
        {/* Pill indicator */}
        <div className={`absolute left-0 w-1 rounded-r-full bg-white transition-all duration-200 ${
          activeServer === 'clawdcord' ? 'h-10' : hoveredServer === 'clawdcord' ? 'h-5' : 'h-0'
        }`} />
        
        <div
          onClick={() => setActiveServer('clawdcord')}
          onMouseEnter={() => setHoveredServer('clawdcord')}
          onMouseLeave={() => setHoveredServer(null)}
          className={`w-12 h-12 flex items-center justify-center cursor-pointer transition-all duration-200 overflow-hidden ${
            activeServer === 'clawdcord'
              ? 'bg-[#5865f2] rounded-2xl'
              : 'bg-[#313338] rounded-[24px] hover:rounded-2xl hover:bg-[#5865f2]'
          }`}
        >
        ...
        {/* Tooltip */}
        <div className={`absolute left-full ml-4 px-3 py-2 bg-[#111214] text-white text-sm font-semibold rounded-md shadow-lg whitespace-nowrap z-50 pointer-events-none transition-opacity duration-100 ${
          hoveredServer === 'clawdcord' ? 'opacity-100' : 'opacity-0'
        }`}>
          Clawdcord
          <div className="absolute left-0 top-1/2 -translate-x-1 -translate-y-1/2 border-4 border-transparent border-r-[#111214]" />
        </div>
      ...
      <div className="relative flex items-center justify-center mb-2 group">
        <div
          ...
          onMouseLeave={() => setHoveredServer(null)}
          className="w-12 h-12 bg-[#313338] rounded-[24px] hover:rounded-2xl hover:bg-[#23a559] flex items-center justify-center cursor-pointer transition-all duration-200 text-[#23a559] hover:text-white"
        >
        ...
        {/* Tooltip */}
        <div className={`absolute left-full ml-4 px-3 py-2 bg-[#111214] text-white text-sm font-semibold rounded-md shadow-lg whitespace-nowrap z-50 pointer-events-none transition-opacity duration-100 ${
          hoveredServer === 'add' ? 'opacity-100' : 'opacity-0'
        }`}>
          Add a Server
          <div className="absolute left-0 top-1/2 -translate-x-1 -translate-y-1/2 border-4 border-transparent border-r-[#111214]" />
        </div>
      ...
      <div className="relative flex items-center justify-center group">
        ...
        {/* Tooltip */}
        <div className={`absolute left-full ml-4 px-3 py-2 bg-[#111214] text-white text-sm font-semibold rounded-md shadow-lg whitespace-nowrap z-50 pointer-events-none transition-opacity duration-100 ${
          hoveredServer === 'explore' ? 'opacity-100' : 'opacity-0'
        }`}>
