...
export default function ServerList() {
  ...
  const getIndicatorHeight = (id: string) => {
    if (activeServer === id) return 'h-10';
    if (hoveredServer === id) return 'h-5';
    return 'h-0';
  };

  return (
    <nav className="w-[72px] bg-[#1e1f22] flex flex-col items-center pt-3 pb-3 flex-shrink-0 h-full overflow-y-auto overflow-x-hidden">
      {/* Home Button */}
      <div className="relative flex items-center justify-center w-full px-3 mb-2">
        {/* Pill Indicator */}
        <span 
          className={`absolute left-0 w-1 bg-white rounded-r-full pill-indicator ${getIndicatorHeight('home')}`}
        />
        ...
        <button
          ...
          className={`w-12 h-12 flex items-center justify-center server-icon ${
            activeServer === 'home' 
              ? 'bg-[#5865f2] rounded-[16px]' 
              : 'bg-[#313338] rounded-[24px] hover:bg-[#5865f2]'
          }`}
        >
      ...
      {/* Divider */}
      <div className="w-8 h-0.5 bg-[#35373c] rounded-full mb-2" />

      {/* Clawdcord Server */}
      <div className="relative flex items-center justify-center w-full px-3 mb-2">
        <span 
          className={`absolute left-0 w-1 bg-white rounded-r-full pill-indicator ${getIndicatorHeight('clawdcord')}`}
        />
        ...
        <button
          ...
          onMouseLeave={() => setHoveredServer(null)}
          className={`w-12 h-12 flex items-center justify-center server-icon overflow-hidden ${
      ...
      <div className="relative flex items-center justify-center w-full px-3 mb-2">
        <button
          ...
          onMouseLeave={() => setHoveredServer(null)}
          className="w-12 h-12 bg-[#313338] rounded-[24px] flex items-center justify-center server-icon text-[#23a559] hover:bg-[#23a559] hover:text-white"
        >
